...
page_init(void) {
 ...
 // end address of kernel
  ...
  cprintf("freemem start at: %08x", freemem);
  ...
  cprintf("free pages: %08x", (mend-mbegin)>>PGSHIFT);
  cprintf("## %08x", sizeof(struct Page));
}
...
get_pte(pde_t *pgdir, uintptr_t la, bool create) {
    ...
    if (!(*pdep & PTE_P)) {
                struct Page* new_pte;
                if(!create|| (new_pte = alloc_page()) == NULL) {
                        return NULL;
                }
        ...
        return  (pte_t*)KADDR((uintptr_t)((pte_t*)(PDE_ADDR(*pdep))+PTX(la)));// return page table entry
}
